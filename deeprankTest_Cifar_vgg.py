...
# Define VGG-based model for feature extraction
class VGGModel(nn.Module):
    def __init__(self):
        super(VGGModel, self).__init__()
        self.vgg = models.vgg16(pretrained=True)
        self.vgg.classifier = nn.Sequential(*list(self.vgg.classifier.children())[:-1])  # Remove the final layer
        self.embedding_layer = nn.Linear(4096, 128)  # You can change embedding size as needed
...
def calculate_similarity_and_save(model, image_pairs, save_path='cosine_sim/cosine_sim_deeprank_vgg_cifar10.npy'):
    cosine_sim = []
...
if __name__ == "__main__":
    ...
    model_path = "vgg_triplet_model.pth"
