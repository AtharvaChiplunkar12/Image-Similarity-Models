...
# Function to create base network with VGG16 pre-trained on ImageNet without fine-tuning
def create_base_network(input_shape):
    # Load the pre-trained VGG16 model (no top layers)
    base_model = VGG16(weights='imagenet', input_shape=input_shape, include_top=False)  # Using pre-trained weights from ImageNet
...
input_shape = (224, 224, 3)  # Resize CIFAR-10 images to (224, 224, 3) for VGG16
...
# Function to preprocess a single image for VGG16
def preprocess_image(img):
    # Resize to VGG input shape (224, 224)
...
def predict_similarity_for_pairs(image_pairs_test):
    ...
    save_path = 'cosine_sim/cosine_sim_siameseNet_cifar10_vgg.npy'
